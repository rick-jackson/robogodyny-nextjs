...
export default function RoboGodynyApp() {
  // State
  const [currentUser, setCurrentUser] = useState<'user' | 'admin'>('user');
  ...
  useEffect(() => {
    const saved = localStorage.getItem('robogodyny_entries');
    if (saved) {
  ...
  const saveEntries = (newEntries: WorkEntry[]) => {
    ...
    localStorage.setItem('robogodyny_entries', JSON.stringify(newEntries));
  };
  ...
  const handleSaveEntry = (entryData: any) => {
    let newEntries: WorkEntry[];

    if (editingEntry) {
      // Update
      newEntries = entries.map(e => 
        e.id === editingEntry.id ? { ...e, ...entryData } as WorkEntry : e
      );
    } else {
      // Add new (remove old if exists for same date to avoid duplicates)
      const filtered = entries.filter(e => e.date !== entryData.date);
      const newEntry: WorkEntry = {
        id: Date.now(),
        ...entryData,
        createdAt: new Date().toISOString(),
      };
      newEntries = [...filtered, newEntry];
    }
  ...
  const handleDeleteEntry = () => {
    ...
    if (confirm('Видалити цей запис?')) {
      const newEntries = entries.filter(e => e.id !== detailEntry.id);
      saveEntries(newEntries);
      setShowDetailModal(false);
      setDetailEntry(null);
  ...
  const switchUser = (type: 'user' | 'admin') => {
    setCurrentUser(type);
    if (type === 'admin') {
      setSelectedAdminUser(null);
    }
  ...
  const showToast = (message: string) => {
    ...
    toast.className = `fixed bottom-6 left-1/2 -translate-x-1/2 bg-zinc-800 border border-zinc-700 text-white px-6 py-3 rounded-3xl flex items-center gap-x-3 text-sm shadow-xl z-[100]`;
    toast.innerHTML = `
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className="text-2xl font-semibold tracking-tighter">{day}</div>
              ...
              <div className="lg:col-span-2 bg-zinc-900 border border-zinc-800 rounded-3xl p-6 min-h-[420px]">
                {!selectedAdminUser ? (
                  <div className="text-center py-16 text-zinc-500">
                    ...
                    <p>Оберіть співробітника зе списку</p>
                  </div>
                ) : (
                  <div>
                    {/* User Header */}
                    <div className="flex items-center gap-x-4 mb-6">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-xl tracking-tighter">{selectedAdminUser.name}</div>
